...
| 2 | **Tier C (Makro/Game/Incredible/HiFi Corp/Amazon) cannot be end-to-end verified.** They are expected to be blocked from a home IP. Unit tests prove the parser degrades to a logged `blocked` (challenge-page detection + no-product-markup default) and still extracts a price on the rare page that gets through — but no real fetch is exercised. The challenge-marker list (`CHALLENGE_MARKERS` in `bestEffortScraper.ts`) is heuristic and may miss a new anti-bot vendor's wording. | Anti-bot evasion (proxies/stealth) is explicitly out of scope for the MVP; a Tier C miss is acceptable and logged, never a crash. | **If/when Tier C reliability is ever needed** (post-MVP, would require proxy/stealth infra — a separate decision, see Risk Register). Until then, extend `CHALLENGE_MARKERS` only if a Tier C store starts returning a 200 challenge page that slips through as `parse_error` in `scrape_errors`. |
